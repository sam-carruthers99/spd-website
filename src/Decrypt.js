
export default function Encrypt(name, inputText, inputKey, inputKey2){
    switch(name){
        case "Caesar":
            return DecryptCaesar(inputText, inputKey)
        case "Atbash":
            return DecryptAtbash(inputText)
        case "Vigenere":
            return DecryptVigenere(inputText, inputKey)
        case "ADFGVX":
            return DecryptCaesar(inputText, inputKey) // not done
        case "Bifid":
            return DecryptBifid(inputText, inputKey)
        case "Four Square":
            return DecryptVigenere(inputText, inputKey) // not done
        case "ADFGX":
            return DecryptCaesar(inputText, inputKey) // not done
        case "Trifid":
            return DecryptAtbash(inputText) // not done
        case "One Time Pad":
            return DecryptOTP(inputText, inputKey)
        case "Rail Fence":
            return DecryptRailFence(inputText, inputKey, inputKey2)
        case "Columnar Transposition":
            return DecryptColumnarTransposition(inputText, inputKey)
        case "Playfair":
            return DecryptPlayfair(inputText, inputKey)
        default:
            break;
    }
}

function DecryptCaesar(inputText, inputKey){

    const text = inputText.toUpperCase();
    const key = parseInt(inputKey)

    // Initialize an empty string to store the decrypted text
    let decryptedText = '';

    // Iterate through each character in the encrypted text
    for (let i = 0; i < text.length; i++) {
        const char = text[i];

        // Check if the character is a letter
        if (char.match(/[A-Z]/)) {
            // Get the ASCII code of the character
            const charCode = char.charCodeAt(0);

            // Apply the Caesar cipher decryption with the specified key
            const decryptedCharCode = ((charCode - 65 - key + 26) % 26) + 65;

            // Convert the decrypted ASCII code back to a character and append to the result
            decryptedText += String.fromCharCode(decryptedCharCode);
        } else {
            // If the character is not a letter, leave it unchanged
            decryptedText += char;
        }
    }

    return decryptedText;
}

function DecryptVigenere(cipherText, inputKey) {
    cipherText = cipherText.toUpperCase();
    inputKey = inputKey.toUpperCase();

    let decryptedText = "";
    let keyIndex = 0;

    for (let i = 0; i < cipherText.length; i++) {
        let charCode = cipherText.charCodeAt(i);

        if (charCode >= 65 && charCode <= 90) {
            let keyCharCode = inputKey.charCodeAt(keyIndex % inputKey.length);
            let decryptedCharCode = ((charCode - 65 - (keyCharCode - 65) + 26) % 26) + 65;
            decryptedText += String.fromCharCode(decryptedCharCode);
            keyIndex++;
        } else {
            decryptedText += cipherText[i];
        }
    }

    return decryptedText;
}

function DecryptAtbash(inputText) {
    const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    const reversedAlphabet = 'ZYXWVUTSRQPONMLKJIHGFEDCBA';

    let decryptedText = '';

    for (let char of inputText.toUpperCase()) {
        const index = reversedAlphabet.indexOf(char);
        if (index !== -1) {
            decryptedText += alphabet[index];
        } else {
            decryptedText += char;
        }
    }

    return decryptedText;
}

function createKeySquare(key) {
    key = key.toUpperCase().replace(/[^A-Z]/g, '');
    let keySquare = '';
    for (let char of key + 'ABCDEFGHIKLMNOPQRSTUVWXYZ') {
        if (!keySquare.includes(char) && char !== 'J') {
            keySquare += char;
        }
    }
    return keySquare;
}

function getCoordinates(text, keySquare) {
    const coordinates = [];
    for (let char of text) {
        if (char === 'J') char = 'I';
        const index = keySquare.indexOf(char);
        if (index !== -1) {
            coordinates.push([Math.floor(index / 5), index % 5]);
        }
    }
    return coordinates;
}

function flattenCoordinates(coordinates) {
    return coordinates.reduce((acc, val) => acc.concat(val), []);
}

function transposeCoordinates(flattenedCoordinates) {
    const halfLength = Math.ceil(flattenedCoordinates.length / 2);
    const firstHalf = flattenedCoordinates.slice(0, halfLength);
    const secondHalf = flattenedCoordinates.slice(halfLength);

    const transposedCoordinates = [];
    for (let i = 0; i < firstHalf.length; i++) {
        transposedCoordinates.push([firstHalf[i], secondHalf[i]]);
    }
    return transposedCoordinates;
}

function getTextFromCoordinates(transposedCoordinates, keySquare) {
    let text = '';
    for (let pair of transposedCoordinates) {
        if (pair[0] !== undefined && pair[1] !== undefined) {
            text += keySquare[pair[0] * 5 + pair[1]];
        }
    }
    return text;
}

function splitFlattenedCoordinates(flattenedCoordinates) {
    const halfLength = Math.ceil(flattenedCoordinates.length / 2);
    return [
        flattenedCoordinates.slice(0, halfLength),
        flattenedCoordinates.slice(halfLength)
    ];
}

function transposeCoordinatesBack(splitCoordinates) {
    const [firstHalf, secondHalf] = splitCoordinates;
    const transposedCoordinates = [];

    for (let i = 0; i < firstHalf.length; i++) {
        transposedCoordinates.push([firstHalf[i], secondHalf[i]]);
    }
    return transposedCoordinates;
}

function DecryptBifid(inputText, inputKey) {
    const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    const keySquare = createKeySquare(inputKey);
    const coordinates = getCoordinates(inputText.toUpperCase(), keySquare);
    const flattenedCoordinates = flattenCoordinates(coordinates);
    const splitCoordinates = splitFlattenedCoordinates(flattenedCoordinates);
    const originalCoordinates = transposeCoordinatesBack(splitCoordinates);
    const decryptedText = getTextFromCoordinates(originalCoordinates, keySquare);

    return decryptedText;
}

function DecryptOTP(encryptedText, inputKey) {
    const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    let sanitizedKey = inputKey.replace(/\s+/g, '').toUpperCase();
    let decryptedText = '';
    let keyIndex = 0;

    for (let char of encryptedText.toUpperCase()) {
        if (char === ' ') {
            decryptedText += ' ';
        } else {
            const encryptedChar = char;
            const keyChar = sanitizedKey.charAt(keyIndex);

            if (alphabet.includes(encryptedChar) && alphabet.includes(keyChar)) {
                const encryptedIndex = alphabet.indexOf(encryptedChar);
                const keyIndexValue = alphabet.indexOf(keyChar);
                const decryptedIndex = (encryptedIndex - keyIndexValue + 26) % 26;
                decryptedText += alphabet.charAt(decryptedIndex);
                keyIndex = (keyIndex + 1) % sanitizedKey.length;
            } else {
                decryptedText += encryptedChar;
            }
        }
    }

    return decryptedText;
}

function DecryptColumnarTransposition(encryptedText, inputKey) {
    const sanitizedKey = inputKey.replace(/\s+/g, '').toUpperCase();
    const numCols = sanitizedKey.length;
    const numRows = Math.ceil(encryptedText.length / numCols);
    
    let sortedKeyIndices = [...sanitizedKey].map((char, index) => [char, index])
                                            .sort(([a], [b]) => a.localeCompare(b))
                                            .map(([, index]) => index);
    
    let grid = Array.from({ length: numRows }, () => Array(numCols).fill(''));
    let currentIndex = 0;

    for (let colIndex of sortedKeyIndices) {
        for (let row = 0; row < numRows; row++) {
            if (currentIndex < encryptedText.length) {
                grid[row][colIndex] = encryptedText[currentIndex++];
            }
        }
    }

    let decryptedText = '';
    for (let row = 0; row < numRows; row++) {
        for (let col = 0; col < numCols; col++) {
            if (grid[row][col]) {
                decryptedText += grid[row][col];
            }
        }
    }

    return decryptedText;
}

function generatePlayfairMatrix(key) {
    const alphabet = 'ABCDEFGHIKLMNOPQRSTUVWXYZ';
    const keySquare = [];
    const usedChars = new Set();
    
    for (let char of key + alphabet) {
        if (!usedChars.has(char)) {
            usedChars.add(char);
            keySquare.push(char);
        }
    }
    
    const matrix = [];
    for (let i = 0; i < 5; i++) {
        matrix.push(keySquare.slice(i * 5, i * 5 + 5));
    }
    
    return matrix;
}

function createPairs(text) {
    const pairs = [];
    for (let i = 0; i < text.length; i += 2) {
        let first = text[i];
        let second = text[i + 1] || 'X';
        if (first === second) {
            second = 'X';
            i--;
        }
        pairs.push([first, second]);
    }
    return pairs;
}

function findPosition(matrix, char) {
    for (let row = 0; row < 5; row++) {
        for (let col = 0; col < 5; col++) {
            if (matrix[row][col] === char) {
                return [row, col];
            }
        }
    }
    return [-1, -1];
}

function DecryptPlayfair(encryptedText, inputKey) {
    const sanitizedText = encryptedText.toUpperCase().replace(/[^A-Z]/g, '');
    const sanitizedKey = inputKey.toUpperCase().replace(/J/g, 'I').replace(/[^A-Z]/g, '');
    
    const matrix = generatePlayfairMatrix(sanitizedKey);
    const pairs = createPairs(sanitizedText);
    
    let decryptedText = '';
    for (let [first, second] of pairs) {
        const [firstRow, firstCol] = findPosition(matrix, first);
        const [secondRow, secondCol] = findPosition(matrix, second);
        
        if (firstRow === secondRow) {
            decryptedText += matrix[firstRow][(firstCol + 4) % 5] + matrix[secondRow][(secondCol + 4) % 5];
        } else if (firstCol === secondCol) {
            decryptedText += matrix[(firstRow + 4) % 5][firstCol] + matrix[(secondRow + 4) % 5][secondCol];
        } else {
            decryptedText += matrix[firstRow][secondCol] + matrix[secondRow][firstCol];
        }
    }
    
    return decryptedText;
}

function offsetText(text, offset) {
    return text.slice(offset) + text.slice(0, offset);
}

function DecryptRailFence(encryptedText, inputKey, inputKey2) {
    const numRails = parseInt(inputKey, 10);
    const offset = parseInt(inputKey2, 10);

    if (numRails <= 1) {
        return encryptedText;
    }

    const correctedText = offsetText(encryptedText, -offset);
    const railLengths = Array(numRails).fill(0);
    let directionDown = false;
    let row = 0;

    for (let i = 0; i < correctedText.length; i++) {
        railLengths[row]++;
        if (row === 0 || row === numRails - 1) {
            directionDown = !directionDown;
        }
        row += directionDown ? 1 : -1;
    }

    const railFence = [];
    let currentIndex = 0;

    for (let i = 0; i < numRails; i++) {
        railFence.push(correctedText.slice(currentIndex, currentIndex + railLengths[i]).split(''));
        currentIndex += railLengths[i];
    }

    let decryptedText = '';
    directionDown = false;
    row = 0;

    for (let i = 0; i < correctedText.length; i++) {
        decryptedText += railFence[row].shift();
        if (row === 0 || row === numRails - 1) {
            directionDown = !directionDown;
        }
        row += directionDown ? 1 : -1;
    }

    return decryptedText;
}
